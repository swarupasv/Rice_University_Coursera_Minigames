...
def draw(canvas):
    global score1, score2, paddle1_pos, paddle2_pos, ball_pos, ball_vel, paddle1_vel, paddle2_vel, PAD_VELOCITY
    ...
    # draw ball
    canvas.draw_circle((ball_pos), BALL_RADIUS, BALL_RADIUS*2, 'White')
    
    # update paddle's vertical position, keep paddle on the screen
    if paddle1_vel < 0 and paddle1_pos >= 0:
        paddle1_pos += paddle1_vel
    if paddle1_vel > 0 and paddle1_pos <= HEIGHT - PAD_HEIGHT:
        paddle1_pos += paddle1_vel
        
    if paddle2_vel < 0 and paddle2_pos >= 0:
        paddle2_pos += paddle2_vel
    if paddle2_vel > 0 and paddle2_pos <= HEIGHT - PAD_HEIGHT:
        paddle2_pos += paddle2_vel
    ...
    if ball_pos[0] <= (BALL_RADIUS + PAD_WIDTH) or ball_pos[0] >= (WIDTH - PAD_WIDTH - BALL_RADIUS):        
        ...
        if (ball_vel[0] > 0):
            if (ball_pos[1] < paddle1_pos or ball_pos[1] > paddle1_pos + PAD_HEIGHT ):
                spawn_ball(RIGHT)
                score2 += 1
            else: ball_vel[0] += ball_vel[0] * VELOCITY_INCREASE
    ...
    canvas.draw_text(str(score2), (WIDTH/4*3, 50), 30, 'White')
