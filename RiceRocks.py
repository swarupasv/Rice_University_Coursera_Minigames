...
class Sprite:
    ...
    def collide(self,other_object):
        r1=other_object.get_radius()
        r2=self.radius
        c1=other_object.get_position()
        c2=self.pos
        d=dist(c1,c2)
        if d>r1+r2:
            return False
...
def group_collide(rocks,other_object):
        s=set([])
        global lives,started,timer,rock_group,score,explosion_group
        for a_rock in rocks:
            if a_rock.collide(other_object):
                explosion_group.add (Sprite (a_rock.pos, [0, 0], 0, 0, explosion_image, explosion_info))
...
def process_sprite_group(missile_group,canvas):
    remove=set([])
    for a_missile in missile_group:
        a_missile.draw(canvas)
        if a_missile.update():
            remove.add(a_missile)
    for a_missile in remove:
        missile_group.remove(a_missile)
        
        
def process_rock_group(rock_group,canvas):
    remove=set([])
    for a in rock_group:
        a.draw(canvas)
        if a.update():
...
def click(pos):
    ...
    if not started:
        soundtrack.pause() 
def draw(canvas):
    global time, started,rock_group,my_ship,missile_group
    ...
    process_rock_group(rock_group,canvas)
